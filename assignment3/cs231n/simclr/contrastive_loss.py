...
def sim(z_i, z_j):
    ...
    norm_dot_product =  (z_i @ z_j) / (z_i_norm * z_j_norm)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        denominator1 = sum(torch.exp(sim(z_k, out[i]) / tau) for i in range(2*N) if i != k)
        denominator2 = sum(torch.exp(sim(z_k_N, out[i]) / tau) for i in range(2*N) if i != k+N)
...
def sim_positive_pairs(out_left, out_right):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    norms_left = torch.linalg.norm(out_left, dim=1, keepdims=True)
    norms_right = torch.linalg.norm(out_right, dim=1, keepdims=True)

    pos_pairs = torch.sum(out_left * out_right, axis=1, keepdim=True) / (norms_left * norms_right)
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
...
def compute_sim_matrix(out):
    ...
    out_norms = out / torch.linalg.norm(out, dim=1, keepdims=True)
    sim_matrix = out_norms @ out_norms.T
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    loss1 = -torch.log(numerator / denom[:N])
    loss2 = -torch.log(numerator / denom[N:])
    loss = torch.sum(loss1 + loss2) / (2*N)
