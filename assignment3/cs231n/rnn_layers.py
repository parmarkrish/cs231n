...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    h[:,0,:], caches['c0'] = rnn_step_forward(x[:,0,:], h0, Wx, Wh, b)

    for t in range(1,T):
      h[:,t,:], caches['c'+str(t)] = rnn_step_forward(x[:,t,:], h[:,t-1,:], Wx, Wh, b)
...
def rnn_backward(dh, cache):
    ...
    caches = cache
    N, T, H = dh.shape
    D, _ = caches['c0'][4].shape

    # Setting the shape of arrays
    dx = np.zeros((N,T,D))
    dh0 = np.zeros((N,H))
    dWx = np.zeros((D,H))
    dWh = np.zeros((H,H))
    ...
    for t in reversed(range(0,T)):
      dx_, dprev_h, dWx_, dWh_, db_ = rnn_step_backward(dh[:,t,:], caches['c'+str(t)])
      dx[:,t,:] += dx_
      dWx += dWx_
      dWh += dWh_
      db += db_
      for n in reversed(range(t)):
        dx_, dprev_h, dWx_, dWh_, db_ = rnn_step_backward(dprev_h, caches['c'+str(n)])
        dx[:,n,:] += dx_
        dWx += dWx_
        dWh += dWh_
        db += db_
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    a = x.dot(Wx) + prev_h.dot(Wh) + b # activations (N, 4H)

    i, f, o, g = np.hsplit(a, 4) # input gate, forget gate, output gate and block input (N, H)

    sigmoid_f = sigmoid(f)
    sigmoid_i = sigmoid(i)
    tanh_g = np.tanh(g)
    next_c = sigmoid_f * prev_c + sigmoid_i * tanh_g # next cell state (N, H)


    tanh_next_c = np.tanh(next_c)
    sigmoid_o = sigmoid(o)
    next_h = sigmoid_o * tanh_next_c # next hidden state (N, H)

    cache = (tanh_next_c, sigmoid_o, prev_c, sigmoid_f, tanh_g, sigmoid_i, Wx, x, Wh, prev_h)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    tanh_next_c, sigmoid_o, prev_c, sigmoid_f, tanh_g, sigmoid_i, Wx, x, Wh, prev_h = cache

    do = dnext_h * tanh_next_c * (sigmoid_o * (1 - sigmoid_o))

    dnext_c_ = dnext_c + dnext_h * sigmoid_o * (1 - tanh_next_c**2)

    df = dnext_c_ * prev_c * (sigmoid_f * (1 - sigmoid_f))

    dprev_c = dnext_c_ * sigmoid_f

    di = dnext_c_ * tanh_g * (sigmoid_i * (1 - sigmoid_i))

    dg = dnext_c_ * sigmoid_i * (1 - tanh_g**2)
...
def lstm_backward(dh, cache):
    ...
    N, T, H = dh.shape
    D = cache[0][6].shape[0] # obtaining Wx D value from a place in cache
    ...
    for i in reversed(range(T)):
      dx[:,i,:], dprev_h, dprev_c, dWx_, dWh_, db_ = lstm_step_backward(dh[:, i, :] + dnext_h, dnext_c, cache[i])
