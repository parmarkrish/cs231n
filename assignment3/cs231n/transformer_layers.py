...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        ind = torch.arange(0, max_len).unsqueeze(1)
        e = torch.exp(torch.arange(0, embed_dim, 2) * -(math.log(10000) / embed_dim))
        pe[:, :, 0::2] = torch.sin(ind * e)
        pe[:, :, 1::2] = torch.cos(ind * e)
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, embed_dim, num_heads, dropout=0.1):
        ...
        self.scaling_factor = math.sqrt(embed_dim / num_heads)
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        # (N, S, E) @ (E, E) -> (N, S, E) -> (N, S, H, D/H) -> (N, H, S, D/H)
        Q = self.query(query).view(N, S, H, D//H).permute(0, 2, 1, 3)
        # (N, T, E) @ (E, E) -> (N, T, E) -> (N, T, H, D/H) -> (N, H, T, D/H)
        ...
        #Y = torch.matmul(self.dropout(F.softmax(Y, -1)), V)
