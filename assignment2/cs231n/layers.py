...
def affine_forward(x, w, b):
    ...
    x_unraveled = x.reshape(x.shape[0], -1)

    out = x_unraveled.dot(w) + b
    ...
    for n in range(N):
      for f in range(F):
        for h_ in range(H_):
          ...
          for w_ in range(W_):
            wf = w_ * stride
            out[n, f, h_, w_] = np.sum(x_pad[n, :C, hf:hf+HH, wf:wf+WW] * w[f]) + b[f]
    ...
    for n in range(N):
      for f in range(F):
        for h_ in range(H_):
          ...
          for w_ in range(W_):
            wf = w_ * stride
            dx_pad[n, :C, hf:hf+HH, wf:wf+WW] += w[f] * dout[n, f, h_, w_]
            dw[f] += x_pad[n, :C, hf:hf+HH, wf:wf+WW] * dout[n, f, h_, w_]
            db[f] += dout[n, f, h_, w_]
...
def max_pool_forward_naive(x, pool_param):
    ...
    for n in range(N):
      for h_ in range(H_):
        ...
        for w_ in range(W_):
          wf = w_ * stride
          out[n, :, h_, w_] = np.amax(x[n, :, hf:hf+pool_height, wf:wf+pool_width], axis=(1,2))
    ...
    for n in range(N):
      for c in range(C):
        for h_ in range(H_):
          ...
          for w_ in range(W_):
            wf = w_ * stride
            ind_flat = np.argmax(x[n, c, hf:hf+pool_height, wf:wf+pool_width])
            ind = np.unravel_index(ind_flat, (pool_height,pool_width))
            dx[n,c,hf:hf+pool_height,wf:wf+pool_height][ind] = dout[n,c,h_,w_]
